...
export const useAuthStore = create((set) => ({ //a one argument and a callback function 
    ...
    signup: async (name, email, phone, roomNumber, moveIn, password) => {
        ...
        try {
            ...
            const response = await axios.post(`${API_URL}/signups`, { name, email, phone, roomNumber, moveIn, password }); //we sending this to the server
    ...
    verifyEmail: async (code) => {
        try {
            const response = awaitaxios.post(`${API_URL}/verify-email`, { code })
