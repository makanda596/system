...
export const useAuthStore = create((set) => ({ //a one argument and a callback function 
    ...
    sendAlert: async (messages) => {
        try {
            const response = await axios.post("http://localhost:5000/auth/alert", { messages })
            set({ messages: response.data.messages })
        } catch (error) {

        }
    },
    fetchAlerts: async (messages) => {
        try {
            const response = await axios.get(`http://localhost:5000/auth/alerts`, messages); // Replace with correct endpoint
            set({ messages: response.data.messages });
        } catch (error) {
            set({
                isLoading: false,
