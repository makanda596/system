...
export const useAuthStore = create((set) => ({ //a one argument and a callback function 
    ...
    //signup part
    signup: async (name, email, phone, roomNumber, isVerified, moveIn, password) => {
        set({ isLoading: true })
        try {

            const response = await axios.post(`${API_URL}/signup`, { name, email, phone, roomNumber, isVerified, moveIn, password }); //we sending this to the server
